...
function install_settings_form(&$form_state, $profile, $install_locale, $settings_file, $db_url, $db_type, $db_prefix, $db_user, $db_pass, $db_host, $db_port, $db_path) {
  ...
  // Modification - CSJ  - 2009-06-10
  // The code has been amended not to allow the user to change the preset values for db name, user and password. 
  // They are displayed as though in input boxes but the values are POSTed
  // to names ending in _display which are not processed.  The preset values are in hidden fields and are POSTed to the correct names.
  // The user is warned above not to change these but should he do so he will confuse only himself.  The preset values are maintained.

    $form['basic_options'] = array(
      '#type' => 'fieldset',
      '#title' => st('Basic options'),
      '#description' => '<p>'. st('Your @drupal database has been already set up with the following information.', array('@drupal' => drupal_install_profile_name())) .
    '<br><b>Do NOT alter these values or you will be unable to access your database </b></p>',
    );
  ...
  /*
    if (count($db_types) > 1) {
      ...
      $db_path_description = st('The name of the database your @drupal data will be stored in. ', array('@drupal' => drupal_install_profile_name()));
    }
    else {
      if (count($db_types) == 1) {
        ...
        $db_path_description = st('The name of the %db_type database your @drupal data will be stored in. ', array('%db_type' => $db_types[0], '@drupal' => drupal_install_profile_name()));
      }
    }
  */
  ...
  $form['basic_options']['db_type'] = array(
    ...
    // Database name
    $form['basic_options']['db_path_display'] = array(
      '#type' => 'textfield',
      '#title' => st('Database name'),
      '#default_value' => 'web160-a-drup-42',
      '#size' => 45,
      '#maxlength' => 45,
      '#required' => TRUE,
  ...
  $form['basic_options']['db_path'] = array(
    '#type' => 'hidden',
    '#value' => 'web160-a-drup-42',
  );

    // Database username
    $form['basic_options']['db_user_display'] = array(
      '#type' => 'textfield',
  ...
  $form['basic_options']['db_user'] = array(
    ...
    // Database password
    $form['basic_options']['db_pass_display'] = array(
      '#type' => 'textfield',
      '#title' => st('Database password'),
      '#default_value' => 'RWHN!hD!d',
      '#size' => 45,
      '#maxlength' => 45,
    );
  $form['basic_options']['db_pass'] = array(
    ...
    // Database host
    $form['advanced_options']['db_host_display'] = array(
      '#type' => 'textfield',
  ...
  $form['basic_options']['db_host'] = array(
    '#type' => 'hidden',
    '#value' => 'localhost',
  );

    // Database port
    $form['advanced_options']['db_port_display'] = array(
      '#type' => 'textfield',
  ...
  $form['basic_options']['db_port'] = array(
    '#type' => 'hidden',
    '#value' => '3306',
  );
    ...
    $form['advanced_options']['db_prefix_display'] = array(
      '#type' => 'textfield',
      '#title' => st('Table prefix'),
      '#default_value' => 'drupal_',
      '#size' => 45,
      '#maxlength' => 45,
      '#description' => st(' '),
    );
  $form['basic_options']['db_prefix'] = array(
    '#type' => 'hidden',
    '#value' => 'drupal_',
  );
...
if (Drupal.jsEnabled) {
  ...
  if ($task == 'finished') {
    // Put the robots.txt file in DocumentRoot 
    system("/home/sites/acecrew.co.uk/public_html/rbell/copy_robots_txt.pl"); 
...
function install_configure_form(&$form_state, $url) {
  ...
  $form['admin_account']['account']['mail'] = array('#type' => 'textfield',
    ...
    '#required' => TRUE,
  '#default_value' => 'admin@acecrew.co.uk', 
    '#weight' => -5,
  );
